...
public class ChinaDailyPredictedController {
    ...
    //数据预测成功直接拿出，不再插入数据库
    @PostMapping("/chinaDailyPredictedOut")
    public Result ChinaDailyPredictedOut(@RequestParam(name = "a1",required = true)double a1, @RequestParam(name = "Sn",required = true) int Sn,
                                      @RequestParam(name = "n",required = true) int n){
        China_daily china_daily = chinaDailyPredictedService.chinaDailyPredictedOut(a1, Sn, n);
        return new Result().setCode(ResultCode.SUCCESS.code()).setMessage("预测成功").setData(china_daily);
    }
